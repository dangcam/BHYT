using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;

namespace BHYT.VO
{
    public class THONGTIN_CTVO
    {
        private string maLK;
        private string maBN;
        private string hoTen;
        private string ngaySinh;
        private int gioiTinh;
        private string diaChi;
        private string maThe;
        private string maDKBD;
        private string gtTheTu;
        private string gtTheDen;
        private string tenBenh;
        private string maBenh;
        private string maBenhKhac;
        private int maLyDoVaoVien;
        private string maNoiChuyen;
        private int maTaiNan;
        private string ngayVao;
        private string ngayRa;
        private int soNgayDieuTri;
        private int ketQuaDieuTri;
        private int tinhTrangRaVien;
        private string ngayThanhToan;
        private int mucHuong;
        private int tienThuoc;
        private int tienVTYT;
        private int tienTongChiPhi;
        private int tienBNTT;
        private int tienBHTT;
        private int tienNguonKhac;
        private int tienNgoaiDS;
        private string namQT;
        private string thangQT;
        private int maLoaiKCB;
        private string maKhoa;
        private string maCSKCB;
        private string maKV;
        private string maPTTTQT;
        private float canNang;
        private bool checkOut;
        private int phong;
        private string maBS;
        public string MaLK
        {
            get
            {
                return maLK;
            }

            set
            {
                maLK = value;
            }
        }

        public string MaBN
        {
            get
            {
                return maBN;
            }

            set
            {
                maBN = value;
            }
        }

        public string HoTen
        {
            get
            {
                return hoTen;
            }

            set
            {
                hoTen = value;
            }
        }

        public string NgaySinh
        {
            get
            {
                return ngaySinh;
            }

            set
            {
                ngaySinh = value;
            }
        }

        public int GioiTinh
        {
            get
            {
                return gioiTinh;
            }

            set
            {
                gioiTinh = value;
            }
        }

        public string DiaChi
        {
            get
            {
                return diaChi;
            }

            set
            {
                diaChi = value;
            }
        }

        public string MaThe
        {
            get
            {
                return maThe;
            }

            set
            {
                maThe = value;
            }
        }

        public string MaDKBD
        {
            get
            {
                return maDKBD;
            }

            set
            {
                maDKBD = value;
            }
        }

        public string GtTheTu
        {
            get
            {
                return gtTheTu;
            }

            set
            {
                gtTheTu = value;
            }
        }

        public string GtTheDen
        {
            get
            {
                return gtTheDen;
            }

            set
            {
                gtTheDen = value;
            }
        }

        public string TenBenh
        {
            get
            {
                return tenBenh;
            }

            set
            {
                tenBenh = value;
            }
        }

        public string MaBenh
        {
            get
            {
                return maBenh;
            }

            set
            {
                maBenh = value;
            }
        }

        public string MaBenhKhac
        {
            get
            {
                return maBenhKhac;
            }

            set
            {
                maBenhKhac = value;
            }
        }

        public int MaLyDoVaoVien
        {
            get
            {
                return maLyDoVaoVien;
            }

            set
            {
                maLyDoVaoVien = value;
            }
        }

        public string MaNoiChuyen
        {
            get
            {
                return maNoiChuyen;
            }

            set
            {
                maNoiChuyen = value;
            }
        }

        public int MaTaiNan
        {
            get
            {
                return maTaiNan;
            }

            set
            {
                maTaiNan = value;
            }
        }

        public string NgayVao
        {
            get
            {
                return ngayVao;
            }

            set
            {
                ngayVao = value;
            }
        }

        public string NgayRa
        {
            get
            {
                return ngayRa;
            }

            set
            {
                ngayRa = value;
            }
        }

        public int SoNgayDieuTri
        {
            get
            {
                return soNgayDieuTri;
            }

            set
            {
                soNgayDieuTri = value;
            }
        }

        public int KetQuaDieuTri
        {
            get
            {
                return ketQuaDieuTri;
            }

            set
            {
                ketQuaDieuTri = value;
            }
        }

        public int TinhTrangRaVien
        {
            get
            {
                return tinhTrangRaVien;
            }

            set
            {
                tinhTrangRaVien = value;
            }
        }

        public string NgayThanhToan
        {
            get
            {
                return ngayThanhToan;
            }

            set
            {
                ngayThanhToan = value;
            }
        }

        public int MucHuong
        {
            get
            {
                return mucHuong;
            }

            set
            {
                mucHuong = value;
            }
        }

        public int TienThuoc
        {
            get
            {
                return tienThuoc;
            }

            set
            {
                tienThuoc = value;
            }
        }

        public int TienVTYT
        {
            get
            {
                return tienVTYT;
            }

            set
            {
                tienVTYT = value;
            }
        }

        public int TienTongChiPhi
        {
            get
            {
                return tienTongChiPhi;
            }

            set
            {
                tienTongChiPhi = value;
            }
        }

        public int TienBNTT
        {
            get
            {
                return tienBNTT;
            }

            set
            {
                tienBNTT = value;
            }
        }

        public int TienBHTT
        {
            get
            {
                return tienBHTT;
            }

            set
            {
                tienBHTT = value;
            }
        }

        public int TienNguonKhac
        {
            get
            {
                return tienNguonKhac;
            }

            set
            {
                tienNguonKhac = value;
            }
        }

        public int TienNgoaiDS
        {
            get
            {
                return tienNgoaiDS;
            }

            set
            {
                tienNgoaiDS = value;
            }
        }

        public string NamQT
        {
            get
            {
                return namQT;
            }

            set
            {
                namQT = value;
            }
        }

        public string ThangQT
        {
            get
            {
                return thangQT;
            }

            set
            {
                thangQT = value;
            }
        }

        public int MaLoaiKCB
        {
            get
            {
                return maLoaiKCB;
            }

            set
            {
                maLoaiKCB = value;
            }
        }

        public string MaKhoa
        {
            get
            {
                return maKhoa;
            }

            set
            {
                maKhoa = value;
            }
        }

        public string MaCSKCB
        {
            get
            {
                return maCSKCB;
            }

            set
            {
                maCSKCB = value;
            }
        }

        public string MaKV
        {
            get
            {
                return maKV;
            }

            set
            {
                maKV = value;
            }
        }

        public string MaPTTTQT
        {
            get
            {
                return maPTTTQT;
            }

            set
            {
                maPTTTQT = value;
            }
        }

        public float CanNang
        {
            get
            {
                return canNang;
            }

            set
            {
                canNang = value;
            }
        }

        public bool CheckOut
        {
            get
            {
                return checkOut;
            }

            set
            {
                checkOut = value;
            }
        }

        public int Phong
        {
            get
            {
                return phong;
            }

            set
            {
                phong = value;
            }
        }

        public string MaBS
        {
            get
            {
                return maBS;
            }

            set
            {
                maBS = value;
            }
        }
    }
}
